...
const ScrollStack = ({
  ...
  const originalInnerBottomRef = useRef(0);
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const cardTop = scrollerAbsoluteTop + originalOffsetsRef.current[i];
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jCardTop = scrollerAbsoluteTop + originalOffsetsRef.current[j];
  ...
  useLayoutEffect(() => {
    ...
    // Measure original offsets BEFORE applying transforms
    cards.forEach(card => {
      card.style.transform = 'none';
    });

    const scrollerRect = scrollerRef.current.getBoundingClientRect();
    const scrollerAbsoluteTop = scrollerRect.top + window.scrollY;

    const relativeOffsets = cards.map(card => {
      const rect = card.getBoundingClientRect();
      const absoluteTop = rect.top + window.scrollY;
      return absoluteTop - scrollerAbsoluteTop;
    });
    originalOffsetsRef.current = relativeOffsets;

    const innerElement = scrollerRef.current?.querySelector('.scroll-stack-inner');
    if (innerElement) {
      originalInnerBottomRef.current = getElementOffset(innerElement) + innerElement.offsetHeight;
    } else {
      originalInnerBottomRef.current = 0;
    }
